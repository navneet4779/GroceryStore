...
const EditSubCategory = ({ data, close, fetchData }) => {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            } else if (uploadResponse && uploadResponse.data && uploadResponse.data.data && uploadResponse.data.data.url) {
                setFormData((prev) => ({
                    ...prev,
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.name || !formData.image || formData.category.length === 0) {
            toast.error("Please fill all fields: Name, Image, and select a Parent Category.");
            return;
        }
        setSaving(true);
        try {
            const payload = {
                ...
                category: formData.category.map(cat => cat.id)
            };
            ...
            if (responseData.success) {
                toast.success(responseData.message || "Subcategory updated");
                if (close) close();
    ...
    const isSubmitDisabled = !formData.name || !formData.image || formData.category.length === 0 || saving || imageUploading;
        ...
        <section className='fixed inset-0 p-4 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300 ease-in-out'>
            <div className='bg-white w-full max-w-2xl rounded-xl shadow-2xl transform transition-all duration-300 ease-in-out scale-100 max-h-[90vh] flex flex-col'>
                <div className='flex items-center justify-between p-4 sm:p-6 border-b border-slate-200 sticky top-0 bg-white rounded-t-xl z-10'>
                    <h2 className='text-xl font-bold text-slate-700'>Edit Subcategory</h2>
                    <button
                        onClick={close}
                ...
                <form className='p-4 sm:p-6 space-y-6 overflow-y-auto flex-grow' onSubmit={handleSubmit}>
                    ...
                    <div className='space-y-1'>
                        ...
                        <div className='flex flex-col sm:flex-row items-center gap-4'>
                            <div className='w-full sm:w-32 h-32 border-2 border-dashed border-slate-300 bg-slate-50 flex items-center justify-center rounded-lg text-slate-400 overflow-hidden'>
                                {formData.image ? (
                                    <img alt='Subcategory Preview' src={formData.image} className='w-full h-full object-contain' />
                                ) : imageUploading ? (
                            ...
                            <label htmlFor='uploadSubCategoryImage' className={`w-full sm:w-auto ${!formData.name || imageUploading || saving ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                                <div className={`py-2.5 px-5 rounded-lg font-medium text-sm flex items-center justify-center gap-2 transition-colors border ${!formData.name || imageUploading || saving ? "bg-slate-200 text-slate-500 border-slate-300" : "bg-sky-500 hover:bg-sky-600 text-white border-sky-500 hover:border-sky-600"}`}>
                            ...
                            {formData.category.length > 0 && (
                                <div className='flex flex-wrap gap-2 mb-2'>
                                    {formData.category.map((cat) => (
                                        <span key={cat.id + "selected"} className='bg-sky-100 text-sky-700 text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5'>
                                            {cat.name}
                                            <button
                                                type="button"
                    ...
                    <div className="pt-4">
                        ...
                        >
                            {saving ? "Updating Subcategory..." : "Update Subcategory"}
                        </button>
                    </div>
                </form>
            </div>
